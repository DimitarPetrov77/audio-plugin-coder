...
//==============================================================================
// Enum parsers � called once per updateLogicBlocks (message thread), so
// processBlock uses integer comparisons instead of ~100 juce::String ==
// "literal" per call (H4 fix).
//==============================================================================
using P = HostesaAudioProcessor;

P::BlockMode P::parseBlockMode(const juce::String &s) {
  if (s == "randomize")
    return BlockMode::Randomize;
  if (s == "envelope")
    return BlockMode::Envelope;
  if (s == "sample")
    return BlockMode::Sample;
  if (s == "morph_pad")
    return BlockMode::MorphPad;
  if (s == "shapes")
    return BlockMode::Shapes;
  if (s == "shapes_range")
...
HostesaAudioProcessor::HostesaAudioProcessor()
  ...
  // Initialize flat param tracking arrays (zero heap allocations on audio
  // thread)
...
HostesaAudioProcessor::createParameterLayout() {
...
//==============================================================================
void HostesaAudioProcessor::prepareToPlay(double sampleRate,
                                          int samplesPerBlock) {
  ...
  // Pre-allocate synth accumulation buffer for layering multiple synths
  // (sequential mode)
  ...
  // Prepare crossover filters, allpass compensation (2N crossovers for N points
  // max)
  ...
  // Purge dead plugin entries (safe: processBlock is not running during
  // prepareToPlay)
  purgeDeadPlugins();
  ...
  for (auto &hp : hostedPlugins) {
    if (hp->instance != nullptr) {
      try {
        ...
        hp->instance->setPlayConfigDetails(pluginIns, pluginOuts, sampleRate,
                                           samplesPerBlock);
        hp->instance->prepareToPlay(sampleRate, samplesPerBlock);
        hp->prepared = true;
      } catch (...) {
        LOG_TO_FILE("prepareToPlay: EXCEPTION for plugin '"
                    << hp->name.toStdString() << "' (ID: " << hp->id
                    << "). Marking crashed.");
        hp->crashed = true;
        hp->prepared = false;
      }
...
void HostesaAudioProcessor::syncProxyCacheToHost() {
  // Drain proxy value cache from audio thread ? call setValueNotifyingHost on
  // message thread. Audio thread writes float values into proxyValueCache
  // atomics; this method reads them.
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool HostesaAudioProcessor::isBusesLayoutSupported(
  ...
  if (!scLayout.isDisabled() && scLayout != juce::AudioChannelSet::mono() &&
...
void HostesaAudioProcessor::updateLogicBlocks(const juce::String &jsonData) {
  ...
  for (int i = 0; i < parsed.size(); ++i) {
    ...
    lb.envFilterMode = obj->hasProperty("envFilterMode")
                           ? obj->getProperty("envFilterMode").toString()
                           : "flat";
    lb.envFilterFreq = obj->hasProperty("envFilterFreq")
                           ? (float)(double)obj->getProperty("envFilterFreq")
                           : 1000.0f;
    lb.envFilterBW = obj->hasProperty("envFilterBW")
                         ? (float)(double)obj->getProperty("envFilterBW")
                         : 2.0f;
    // Convert mode+freq+bw to HPF/LPF cutoffs
    if (lb.envFilterMode == "lp") {
      lb.envBandLo = 20.0f;
      lb.envBandHi = lb.envFilterFreq;
    } else if (lb.envFilterMode == "hp") {
      lb.envBandLo = lb.envFilterFreq;
      lb.envBandHi = 20000.0f;
    } else if (lb.envFilterMode == "bp") {
      lb.envBandLo = lb.envFilterFreq / std::pow(2.0f, lb.envFilterBW * 0.5f);
    ...
    if (lb.loopMode.isEmpty())
      lb.loopMode = "loop";
    if (lb.sampleSpeed <= 0.0f)
      lb.sampleSpeed = 1.0f;
    if (lb.jumpMode.isEmpty())
      lb.jumpMode = "restart";
    ...
    lb.lfoDepth = obj->hasProperty("lfoDepth")
                      ? (float)(double)obj->getProperty("lfoDepth")
                      : 0.8f;
    lb.lfoRotation = obj->hasProperty("lfoRotation")
    ...
    // Circular clamp — ensure playhead is inside the pad (r=0.45)
    {
      ...
      if (pd > 0.45f) {
        float ps = 0.45f / pd;
        lb.playheadX = 0.5f + pdx * ps;
        lb.playheadY = 0.5f + pdy * ps;
      }
    }
    ...
    // Defaults
    if (lb.morphMode.isEmpty())
      lb.morphMode = "manual";
    if (lb.exploreMode.isEmpty())
      lb.exploreMode = "wander";
    if (lb.exploreMode == "lfo")
      lb.exploreMode = "shapes"; // backward compat
    if (lb.lfoShape.isEmpty())
      lb.lfoShape = "circle";
    if (lb.morphAction.isEmpty())
      lb.morphAction = "jump";
    if (lb.stepOrder.isEmpty())
      lb.stepOrder = "cycle";
    if (lb.morphSource.isEmpty())
      lb.morphSource = "midi";
    if (lb.morphGlide <= 0.0f)
      lb.morphGlide = 200.0f;
    if (lb.morphSyncDiv.isEmpty())
      lb.morphSyncDiv = "1/4";
    if (lb.snapRadius <= 0.0f)
      lb.snapRadius = 1.0f;
    ...
    if (lb.shapeType.isEmpty())
      lb.shapeType = "circle";
    if (lb.shapeTracking.isEmpty())
      lb.shapeTracking = "horizontal";
    if (lb.shapeRange.isEmpty())
      lb.shapeRange = "relative";
    if (lb.shapePolarity.isEmpty())
      lb.shapePolarity = "bipolar";
    if (lb.shapeSyncDiv.isEmpty())
      lb.shapeSyncDiv = "1/4";
    if (lb.shapeTrigger.isEmpty())
      lb.shapeTrigger = "free";
    ...
    if (lanesVar.isArray()) {
      for (int li = 0; li < lanesVar.size(); ++li) {
        if (auto *lObj = lanesVar[li].getDynamicObject()) {
          ...
          lc.drift = (lObj->hasProperty("drift")
                          ? (float)(double)lObj->getProperty("drift")
                          : (float)(double)lObj->getProperty("slew")) /
                     50.0f; // -1..+1 (fallback: legacy slew)
          lc.driftRange = lObj->hasProperty("driftRange")
                              ? (float)(double)lObj->getProperty("driftRange")
                              : 5.0f;
          lc.driftScale = lObj->hasProperty("driftScale")
                              ? lObj->getProperty("driftScale").toString()
                              : "1/1";
          ...
          if (lc.loopLen.isEmpty())
            lc.loopLen = "1/1";
          if (lc.interp.isEmpty())
            lc.interp = "smooth";
          if (lc.playMode.isEmpty())
            lc.playMode = "forward";
          if (lc.freeSecs <= 0.0f)
            lc.freeSecs = 4.0f;

          // Oneshot / trigger config
          lc.oneshotMode = (lObj->hasProperty("trigMode")
                                ? lObj->getProperty("trigMode").toString()
                                : "loop") == "oneshot";
          juce::String trigSrc =
              lObj->hasProperty("trigSource")
                  ? lObj->getProperty("trigSource").toString()
                  : "manual";
          lc.trigSourceE = (trigSrc == "midi")    ? 1
                           : (trigSrc == "audio") ? 2
                                                  : 0;
          lc.trigMidiNote = lObj->hasProperty("trigMidiNote")
                                ? (int)lObj->getProperty("trigMidiNote")
                                : -1;
          lc.trigMidiCh = lObj->hasProperty("trigMidiCh")
                              ? (int)lObj->getProperty("trigMidiCh")
                              : 0;
          float threshDb =
              lObj->hasProperty("trigThreshold")
                  ? (float)(double)lObj->getProperty("trigThreshold")
                  : -12.0f;
          lc.trigThresholdLin = std::pow(10.0f, threshDb / 20.0f);
          lc.trigRetrigger = lObj->hasProperty("trigRetrigger")
                                 ? (bool)lObj->getProperty("trigRetrigger")
                                 : true;
          lc.trigHold = lObj->hasProperty("trigHold")
                            ? (bool)lObj->getProperty("trigHold")
                            : false;
          lc.trigAudioSrc = (lObj->hasProperty("trigAudioSrc")
          ...
          // Morph lane mode
          lc.morphMode = lObj->hasProperty("morphMode")
          ...
          if (morphSnapsVar.isArray()) {
            for (int ms = 0; ms < morphSnapsVar.size(); ++ms) {
              if (auto *msObj = morphSnapsVar[ms].getDynamicObject()) {
                ...
                snap.hold = msObj->hasProperty("hold")
                                ? (float)(double)msObj->getProperty("hold")
                                : 0.5f;
                snap.curve = msObj->hasProperty("curve")
                                 ? (int)msObj->getProperty("curve")
                                 : 0;
                snap.depth = msObj->hasProperty("depth")
                                 ? (float)(double)msObj->getProperty("depth")
                                 : 1.0f;
                snap.drift =
                    msObj->hasProperty("drift")
                        ? (float)(double)msObj->getProperty("drift")
                        : (msObj->hasProperty("slew")
                               ? (float)(double)msObj->getProperty("slew")
                               : 0.0f);
                snap.driftRange =
                    msObj->hasProperty("driftRange")
                        ? (float)(double)msObj->getProperty("driftRange")
                        : 5.0f;
                {
                  juce::String dsStr =
                      msObj->hasProperty("driftScale")
                          ? msObj->getProperty("driftScale").toString()
                          : (lObj->hasProperty("driftScale")
                                 ? lObj->getProperty("driftScale").toString()
                                 : "1/1");
                  snap.driftScaleBeats = parseBeatsPerDiv(dsStr);
                }
                snap.warp = msObj->hasProperty("warp")
                                ? (float)(double)msObj->getProperty("warp")
                                : 0.0f;
                snap.steps = msObj->hasProperty("steps")
                                 ? (int)msObj->getProperty("steps")
                                 : 0;
                ...
                if (auto *valsObj = valsVar.getDynamicObject()) {
                  for (auto &prop : valsObj->getProperties()) {
                    snap.values[prop.name.toString().toStdString()] =
            ...
            // -- Pre-parse snapshot values for audio thread (ZERO allocations
            // at RT) -- Convert string keys "pluginId:paramIndex" ? integer
            // pairs. Sort by (pluginId, paramIndex) so all snapshots share the
            // same key order, enabling index-matched iteration on the audio
            // thread.
    ...
    for (const auto &existing : logicBlocks) {
      if (existing.id == lb.id) {
        ...
        bool newRelative =
            (lb.rangeMode == "relative" || lb.shapeRange == "relative");
        bool oldRelative = (existing.rangeMode == "relative" ||
                            existing.shapeRange == "relative");
        ...
        if (lb.laneClips.size() == existing.laneClips.size()) {
          for (size_t li = 0; li < lb.laneClips.size(); ++li) {
            ...
            lb.laneClips[li].oneshotActive =
                existing.laneClips[li].oneshotActive;
  ...
  for (const auto &old : logicBlocks) {
    if (!old.enabled || old.targetBaseValues.empty())
      continue;
    ...
    bool shouldRestore =
        (newLb == nullptr) || (!newLb->enabled) || (newLb->targets.empty());

    if (shouldRestore) {
      for (size_t ti = 0;
           ti < old.targets.size() && ti < old.targetBaseValues.size(); ++ti) {
        ...
        if (slot >= 0 && old.targets[ti].paramIndex < kMaxParams) {
          ...
          paramTouched[slot][old.targets[ti].paramIndex].store(
      ...
      for (size_t ti = 0;
        ...
        for (const auto &nt : newLb->targets) {
          if (nt.pluginId == old.targets[ti].pluginId &&
              nt.paramIndex == old.targets[ti].paramIndex) {
            stillPresent = true;
            break;
          }
        }
  ...
  for (auto &nb : newBlocks) {
    ...
    // If block is new, changed mode, or changed targets ? clear touched for its
    // targets
    bool changed = (oldMatch == nullptr) || (oldMatch->mode != nb.mode) ||
    ...
    if (changed) {
      for (auto &tgt : nb.targets) {
        ...
        if (slot >= 0 && tgt.paramIndex < kMaxParams)
          paramTouched[slot][tgt.paramIndex].store(false,
...
void HostesaAudioProcessor::rebuildPluginSlots() {
  ...
  // Remove old gesture listeners BEFORE destroying them (prevents dangling
  // pointers)
  for (auto &hp : hostedPlugins) {
    if (hp && hp->instance) {
...
void HostesaAudioProcessor::setParamDirect(int pluginId, int paramIndex,
                                           float value) {
  ...
  if (pluginId == kWeqPluginId) {
    ...
    if (paramIndex >= 0 && paramIndex < maxEqBands * 4) {
      ...
      if (band >= 0 && band < numEqPoints.load(std::memory_order_relaxed)) {
        ...
        {
          float targetHz =
              20.0f * std::pow(1000.0f, juce::jlimit(0.0f, 1.0f, value));
          float baseHz = eqPoints[band].freqHz.load(std::memory_order_relaxed);
          eqPoints[band].modFreqHz.store(targetHz - baseHz,
        ...
        case 1: // gainDB (norm 0..1 ? -maxDB..+maxDB dB)
        {
          ...
          float targetDB =
              juce::jlimit(0.0f, 1.0f, value) * maxDB * 2.0f - maxDB;
          float baseDB = eqPoints[band].gainDB.load(std::memory_order_relaxed);
          eqPoints[band].modGainDB.store(targetDB - baseDB,
                                         std::memory_order_relaxed);
          eqPoints[band].modActive.store(true, std::memory_order_relaxed);
          break;
        }
        ...
        case 3: // driftPct (norm 0..1 ? 0..100%)
          eqPoints[band].driftPct.store(juce::jlimit(0.0f, 1.0f, value) *
                                            100.0f,
                                        std::memory_order_relaxed);
    ...
    // -- Global params (100..110): EQ globals --
    else if (paramIndex >= kWeqGlobalBase &&
             paramIndex < kWeqGlobalBase + kWeqGlobalCount) {
      ...
      // have C++ atomics. Modulation for these is applied via JS
      // weqApplyVirtualParam. Setting eqDirty is enough to trigger a re-sync
      // from JS.
      default:
        eqDirty.store(true, std::memory_order_release);
        break;
  ...
  if (pluginId == kWeqPluginId) {
    ...
    if (paramIndex >= 0 && paramIndex < maxEqBands * 4) {
      ...
      if (band >= 0 && band < numEqPoints.load(std::memory_order_relaxed)) {
        ...
        case 0:
          return std::log(
                     eqPoints[band].freqHz.load(std::memory_order_relaxed) /
                     20.0f) /
                 std::log(1000.0f);
        case 1: {
          float maxDB = eqDbRange.load(std::memory_order_relaxed);
          return (eqPoints[band].gainDB.load(std::memory_order_relaxed) +
                  maxDB) /
                 (maxDB * 2.0f);
        }
        case 2:
          return (eqPoints[band].q.load(std::memory_order_relaxed) - 0.025f) /
                 39.975f;
        case 3:
          return eqPoints[band].driftPct.load(std::memory_order_relaxed) /
                 100.0f;
        }
      }
    }
    // Global params (100..113)
      ...
      switch (g) {
      case 0:
        return eqGlobalDepth.load(std::memory_order_relaxed) / 200.0f;
      case 1:
        return (eqGlobalWarp.load(std::memory_order_relaxed) + 100.0f) / 200.0f;
      case 2:
        return (float)eqGlobalSteps.load(std::memory_order_relaxed) / 32.0f;
      case 3:
        return (eqGlobalTilt.load(std::memory_order_relaxed) + 100.0f) / 200.0f;
      // Cases 4-13: JS-side meta params � no C++ atomics. Return 0.5 as safe
      // default.
      default:
        return 0.5f;
      }
    ...
    for (auto &hp : hostedPlugins) {
      if (hp->id == pluginId && hp->instance != nullptr) {
        ...
        if (paramIndex >= 0 && paramIndex < params.size()) {
          ...
          updateParamBase(pluginId, paramIndex,
...
HostesaAudioProcessor::getHostedPluginList() {
  ...
  for (auto &hp : hostedPlugins) {
    ...
    info.numParams =
        hp->instance ? (int)hp->instance->getParameters().size() : 0;
    info.busId = hp->busId;
...
HostesaAudioProcessor::getHostedPluginInstance(int pluginId) {
  std::lock_guard<std::mutex> lock(pluginMutex);
...
void HostesaAudioProcessor::setEqCurve(const juce::String &jsonData) {
  ...
  if (obj->hasProperty("oversample")) {
    ...
    if (newOS != oldOS) {
      ...
      // Reconfigure oversampler (safe: setEqCurve is called from message
      // thread)
      int numCh = getTotalNumOutputChannels();
  ...
  // Dynamic dB range: parse from JS so gain clamping uses the user-selected
  // range
  ...
  // weqParamBase[slot] >= 0 when modulation was recently active (persists
  // across buffers).
  if (obj->hasProperty("globalDepth")) {
    int slot = weqSlot(kWeqGlobalBase + 0);
    if (slot < 0 || weqParamBase[slot] < -0.5f)
      eqGlobalDepth.store(juce::jlimit(
          0.0f, 200.0f, (float)(double)obj->getProperty("globalDepth")));
  }
  // Global warp: S-curve contrast (-100 to +100)
  if (obj->hasProperty("globalWarp")) {
    int slot = weqSlot(kWeqGlobalBase + 1);
    if (slot < 0 || weqParamBase[slot] < -0.5f)
      eqGlobalWarp.store(juce::jlimit(
          -100.0f, 100.0f, (float)(double)obj->getProperty("globalWarp")));
  }
  // Global steps: quantize gain to N discrete levels (0 = continuous, =2 =
  // stepped)
  if (obj->hasProperty("globalSteps")) {
    int slot = weqSlot(kWeqGlobalBase + 2);
    if (slot < 0 || weqParamBase[slot] < -0.5f)
      eqGlobalSteps.store(
          juce::jlimit(0, 64, (int)obj->getProperty("globalSteps")));
  }
  // Global tilt: frequency-dependent gain offset (-100 to +100)
  if (obj->hasProperty("globalTilt")) {
    int slot = weqSlot(kWeqGlobalBase + 3);
    if (slot < 0 || weqParamBase[slot] < -0.5f)
      eqGlobalTilt.store(juce::jlimit(
          -100.0f, 100.0f, (float)(double)obj->getProperty("globalTilt")));
  }

  // If any global EQ parameter changed, set eqDirty so biquad coefficients are
  // recalculated. Note: this triggers BIQUAD recalc but NOT crossover reconfig
  // (gated separately by curveChanged).
  ...
  if (auto *pointsArr = pointsVar.getArray()) {
    ...
    for (int i = 0; i < n; ++i) {
      if (auto *pt = (*pointsArr)[i].getDynamicObject()) {
        ...
        if (typeStr == "lp")
          ...
          ft = 3;
        else if (typeStr == "lshf")
          ft = 4;
        else if (typeStr == "hshf")
          ft = 5;
        ...
        sorted.push_back(
            {(float)(double)pt->getProperty("freqHz"),
             ...
             (int)pt->getProperty("busId"), (bool)pt->getProperty("solo"),
             (bool)pt->getProperty("mute"),
             pt->hasProperty("q") ? (float)(double)pt->getProperty("q")
                                  : 0.707f,
             ft,
             pt->hasProperty("drift") ? (float)(double)pt->getProperty("drift")
                                      : 0.0f,
             pt->hasProperty("preEq") ? (bool)pt->getProperty("preEq") : true,
             pt->hasProperty("stereoMode") ? (int)pt->getProperty("stereoMode")
                                           : 0,
             sl, i});
      }
    }
    ...
    if (!pointDataChanged) {
      for (int i = 0; i < (int)sorted.size() && !pointDataChanged; ++i) {
        ...
        if (std::abs(curFreq - juce::jlimit(20.0f, 20000.0f, sorted[i].hz)) >
                0.5f ||
            std::abs(curGain - juce::jlimit(-maxDB, maxDB, sorted[i].db)) >
                0.05f ||
...
bool HostesaAudioProcessor::loadSampleForBlock(int blockId,
                                               const juce::String &filePath) {
  juce::File file(filePath);
  if (!file.existsAsFile()) {
    LOG_TO_FILE(
        "loadSampleForBlock: file not found: " << filePath.toStdString());
    return false;
  }

  std::unique_ptr<juce::AudioFormatReader> reader(
      audioFileFormatManager.createReaderFor(file));
  if (reader == nullptr) {
    LOG_TO_FILE(
        "loadSampleForBlock: unsupported format: " << filePath.toStdString());
    return false;
  }
  ...
  for (int p = 0;
       p < numPeaks && p * samplesPerPeak < monoBuffer.getNumSamples(); ++p) {
    float peak = 0.0f;
  ...
  sd->durationSeconds =
      (float)reader->lengthInSamples / (float)reader->sampleRate;
  ...
  for (auto &lb : logicBlocks) {
    if (lb.id == blockId) {
      ...
      LOG_TO_FILE("loadSampleForBlock: loaded "
                  << sd->fileName.toStdString() << " (" << sd->durationSeconds
                  << "s, " << sd->buffer.getNumSamples() << " samples)");
      return true;
    }
  }
...
bool HostesaAudioProcessor::hasEditor() const { return true; }

//==============================================================================
const juce::String HostesaAudioProcessor::getName() const {
    ...
    for (auto &hp : hostedPlugins) {
      ...
      if (hp->instance) {
        try {
          ...
          LOG_TO_FILE("getState: exception reading params for plugin '"
                      << hp->name.toStdString() << "'");
        }
      }
    }
...
void HostesaAudioProcessor::setStateInformation(const void *data,
                                                int sizeInBytes) {
  ...
  if (pluginsXml != nullptr) {
    ...
    for (int i = 0; i < proxyParamCount; ++i) {
      ...
      if (proxyParams[i] != nullptr) {
        proxyParams[i]->setDynamicName(juce::String("Slot ") +
                                       juce::String(i + 1));
    ...
    for (auto *plugEl : pluginsXml->getChildIterator()) {
      ...
      try {
        ...
        if (newId < 0) {
          LOG_TO_FILE("State restore: failed to reload plugin from "
        ...
        LOG_TO_FILE("State restore: EXCEPTION loading plugin '"
                    << savedPath.toStdString() << "' (id " << savedId
                    << "). Skipping.");
      }
    }
    ...
    if (routingMode.load() == 2 && uiStateJson.isNotEmpty()) {
      ...
      if (auto *root = parsed.getDynamicObject()) {
        ...
        if (auto *weq = weqVar.getDynamicObject()) {
          ...
          float dbRange = weq->hasProperty("dbRange")
                              ? (float)(double)weq->getProperty("dbRange")
                              : 24.0f;
          eqDbRange.store(juce::jlimit(6.0f, 48.0f, dbRange));
          if (weq->hasProperty("depth"))
            eqGlobalDepth.store(juce::jlimit(
                0.0f, 200.0f, (float)(double)weq->getProperty("depth")));
          if (weq->hasProperty("warp"))
            eqGlobalWarp.store(juce::jlimit(
                -100.0f, 100.0f, (float)(double)weq->getProperty("warp")));
          if (weq->hasProperty("steps"))
            eqGlobalSteps.store(
                juce::jlimit(0, 64, (int)weq->getProperty("steps")));
          if (weq->hasProperty("tilt"))
            eqGlobalTilt.store(juce::jlimit(
                -100.0f, 100.0f, (float)(double)weq->getProperty("tilt")));
          if (weq->hasProperty("reso"))
            eqGlobalReso.store(juce::jlimit(
                0.0f, 100.0f, (float)(double)weq->getProperty("reso")));
          ...
          if (auto *ptsArr = ptsVar.getArray()) {
            ...
            for (int i = 0; i < n; ++i) {
              if (auto *pt = (*ptsArr)[i].getDynamicObject()) {
                ...
                float hz =
                    20.0f * std::pow(1000.0f, juce::jlimit(0.0f, 1.0f, xNorm));

                // Convert normalized y (0=top=+maxDB, 1=bottom=-maxDB) ? dB
                float yNorm = (float)(double)pt->getProperty("y");
                float db = (-maxDB) + (1.0f - juce::jlimit(0.0f, 1.0f, yNorm)) *
                                          (maxDB * 2.0f);
                ...
                pts.push_back(
                    {hz, db,
                     pt->hasProperty("uid") ? (int)pt->getProperty("uid")
                                            : (i + 1),
                     ...
                     pt->hasProperty("q") ? (float)(double)pt->getProperty("q")
                                          : 0.707f,
                     ft,
                     pt->hasProperty("drift")
                         ? (float)(double)pt->getProperty("drift")
                         : 0.0f,
                     pt->hasProperty("preEq") ? (bool)pt->getProperty("preEq")
                                              : true,
                     pt->hasProperty("stereoMode")
                         ? (int)pt->getProperty("stereoMode")
                         : 0,
                     pt->hasProperty("slope") ? (int)pt->getProperty("slope")
                                              : 1});
              }
            }
            ...
            std::sort(pts.begin(), pts.end(),
                      [](const PtRestore &a, const PtRestore &b) {
                        return a.hz < b.hz;
                      });

            numEqPoints.store((int)pts.size());
            for (int i = 0; i < (int)pts.size(); ++i) {
              eqPoints[i].freqHz.store(
                  juce::jlimit(20.0f, 20000.0f, pts[i].hz));
              ...
              eqPoints[i].driftPct.store(
                  juce::jlimit(0.0f, 100.0f, pts[i].drift));
              eqPoints[i].preEq.store(pts[i].preEq);
              eqPoints[i].stereoMode.store(
                  juce::jlimit(0, 2, pts[i].stereoMode));
      ...
      if (hp->instance != nullptr) {
        try {
          ...
          LOG_TO_FILE("resetPluginCrash: Plugin '"
                      << hp->name.toStdString()
                      << "' crashed again during prepareToPlay");
          // Leave crashed = true
        }
      }
      break;
...
void HostesaAudioProcessor::migrateOldPresets() {
  ...
  if (oldGlobal.isDirectory()) {
    for (const auto &f :
         oldGlobal.findChildFiles(juce::File::findFiles, false, "*.json")) {
      auto dest = getChainsDir().getChildFile(f.getFileNameWithoutExtension() +
                                              ".mrchain");
  ...
  if (oldPresets.isDirectory()) {
    for (const auto &dir :
         oldPresets.findChildFiles(juce::File::findDirectories, false)) {
      auto destDir = getSnapshotsDir().getChildFile("Unknown").getChildFile(
          sanitizeForFilename(dir.getFileName()));
  ...
  if (!readme.existsAsFile()) {
    readme.replaceWithText(
        ...
        "                Drop .mrchain files into Chains/_Import/ to import "
        "them.\n\n"
        ...
        "This folder is shared across all plugin formats (VST3, AU, "
        "Standalone).\n");
  }
